    ...
    def get_model(self, sysdescr):
        ...
        if len(matches) > 1:
            raise MultipleMatches(
                ('Multiple switch models matched. Update the model '
                 'configurations to use more precise patterns: %s') % sysdescr)
        return matches[0]
...
class Model(BaseYAMLObject):
    ...
    def lookup_oids(self, oid_name, port):
        ...
        return (oid.rx.format(ifIndex=port), oid.tx.format(ifIndex=port))
