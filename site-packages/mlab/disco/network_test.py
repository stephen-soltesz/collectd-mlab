...
class NetworkTest(unittest.TestCase):
    ...
    def test_get_uplink_mac_impl(self):
        route_file = StringIO.StringIO(_FAKE_ROUTE_TABLE_GOOD)
        arp_file = StringIO.StringIO(_FAKE_ARP_CACHE_GOOD)
        expected_uplink_mac = '00:11:22:33:44:55'

        returned_uplink_mac = network.get_uplink_mac_impl(route_file, arp_file)

        self.assertEqual(returned_uplink_mac, expected_uplink_mac)

    def test_get_uplink_mac_impl_raises_uplink_mac_not_found(self):
        route_file = StringIO.StringIO(_FAKE_ROUTE_TABLE_GOOD)
        arp_file = StringIO.StringIO(_FAKE_ARP_CACHE_BAD)

        with self.assertRaises(network.UplinkMACNotFound):
            network.get_uplink_mac_impl(route_file, arp_file)

    def test_get_uplink_mac_impl_raises_no_default_route(self):
        route_file = StringIO.StringIO(_FAKE_ROUTE_TABLE_BAD)
        arp_file = StringIO.StringIO(_FAKE_ARP_CACHE_GOOD)
