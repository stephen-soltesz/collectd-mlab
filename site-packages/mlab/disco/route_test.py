...
class RouteTest(unittest.TestCase):
    ...
    def test_gateways(self):
        ...
        expected = {'default': {route.AF_INET: ('5.4.3.2', 'eth0')}}

        actual = route.gateways(route_lines)

        self.assertDictEqual(expected, actual)

    def test_gateways_raises_NoDefaultGateway_when_gateway_is_missing(self):
        route_lines = FAKE_ROUTE_TABLE_NO_DEFAULT_ROUTE.splitlines()

        with self.assertRaises(route.NoDefaultGateway):
            route.gateways(route_lines)
